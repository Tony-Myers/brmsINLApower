#' Automatic Data Generator for brms + INLA Simulation
#'
#' Creates a function that generates synthetic data for each simulation replicate
#' based on a \pkg{brms}-style model formula, an effect name, and a GLM family,
#' producing columns and structures compatible with \pkg{INLA}.
#'
#' The returned generator function has signature \code{function(n, effect)}, which
#' produces a dataset of size \code{n} with the target effect set to \code{effect}.
#' It automatically adds grouping factors, predictor variables, random effects,
#' and family-specific response generation (including .Ntrials, .E, .scale columns
#' where required by INLA).
#'
#' @param formula Model formula (may include brms-style random effects).
#' @param effect_name Name of the fixed effect to manipulate.
#' @param family brms family object, e.g., \code{gaussian()}, \code{binomial()}.
#' @param family_args Named list of family-specific arguments, e.g.
#'   \code{theta} for negbinomial, \code{phi} for beta, \code{size} for binomial,
#'   \code{E} for poisson exposure, \code{df} for Student-t, \code{shape} for Weibull.
...
#' @param predictor_means Optional named list of means for continuous predictors.
#' @param predictor_sds Optional named list of SDs for continuous predictors.
#'
#' @return A function \code{function(n, effect)} returning a data.frame of simulated data.
#' @keywords internal
.auto_data_generator <- function(formula,
                                 effect_name,
                                 family = gaussian(),
                                 family_args = list(),
                                 error_sd = 1,
  ...
  # Defaults for family-specific knobs
  ...
  function(n, effect) {
    ...
    for (term in fixed_terms) {
      ...
      if (identical(term, effect_name)) {
        eta <- eta + effect * term_value
      } else {
        coef_val <- rnorm(1L, 0, 0.3)
    ...
    # Response generation per family
